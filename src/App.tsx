...
type ColorKey4 = "volt" | "ice" | "ember" | "magenta";
type ColorKey3 = "volt" | "ice" | "magenta";

const colorMap4: Record<ColorKey4, string> = {
  volt: "var(--volt)", ice: "var(--ice)", ember: "var(--ember)", magenta: "var(--magenta)"
};
const colorMap3: Record<ColorKey3, string> = {
  volt: "var(--volt)", ice: "var(--ice)", magenta: "var(--magenta)"
};
...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .avatar-3d { transition: transform 0.15s ease-out; transform-style: preserve-3d; }
    ...
    .hero-orb-1 { width: 240px; height: 240px; background: rgba(200,255,0,0.22); top: 8%; left: -10%; animation-duration: 12s; }
    .hero-orb-2 { width: 220px; height: 220px; background: rgba(255,61,170,0.24); bottom: 10%; right: -8%; animation-duration: 14s; animation-delay: 1s; }
    .hero-orb-3 { width: 170px; height: 170px; background: rgba(168,216,255,0.22); bottom: 24%; left: 24%; animation-duration: 16s; animation-delay: 0.5s; }
    ...
    .contact-focus-card {
      ...
      border-radius: 12px; padding: 18px 20px; box-shadow: 0 14px 34px rgba(0,0,0,0.14);
    }
    .focus-grid { display: grid; grid-template-columns: repeat(2, minmax(0,1fr)); gap: 14px; }
    .focus-pill {
      display: inline-flex; align-items: center; gap: 8px;
      border: 1px solid rgba(200,255,0,0.22); color: var(--volt);
      padding: 8px 12px; border-radius: 999px;
      font-size: 11px; letter-spacing: 0.08em;
      text-transform: uppercase; font-weight: 800; width: fit-content;
    }
...
const Nav = ({ scrollY, theme, setTheme }: {
  ...
  return (
    <nav style={{
      ...
      <div style={{ maxWidth: 1800, width: "100%", margin: "0 auto", padding: "0 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button type="button" onClick={() => setTheme(theme === "dark" ? "light" : "dark")} className="theme-toggle" aria-label="Toggle theme">
            {theme === "dark" ? <><SunMedium size={14} /> Light</> : <><MoonStar size={14} /> Dark</>}
          </button>
...
const Hero = ({ scrollY }: { scrollY: number }) => {
  ...
  return (
    <section id="hero" className="noise-bg" style={{
      ...
      <div style={{
        position: "absolute", top: "-20%", right: "-10%", width: "55vw", height: "55vw", borderRadius: "50%",
      ...
      <div style={{
        position: "absolute", bottom: "-10%", left: "-15%", width: "40vw", height: "40vw", borderRadius: "50%",
      ...
      <div style={{ maxWidth: 1800, margin: "0 auto", padding: "0 16px", width: "100%", position: "relative", zIndex: 2 }}>
        <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "minmax(0,1.12fr) minmax(520px, 0.88fr)", gap: 48, alignItems: "center" }}>
          ...
          <div style={{ position: "relative", zIndex: 2 }}>
            ...
            <h1 className="animate-in animate-in-d1 hero-title" style={{
              ...
              fontSize: "clamp(64px, 9vw, 124px)", marginBottom: 0, color: "var(--text)"
            }}>
              MD<br />
            ...
            <p className="animate-in animate-in-d2 serif-italic soft-title" style={{
              fontSize: "clamp(18px, 2vw, 24px)", margin: "24px 0 32px", lineHeight: 1.5, maxWidth: 560
            }}>
            ...
            <div className="animate-in animate-in-d4" style={{
              display: "flex", gap: 24, flexWrap: "nowrap", justifyContent: "space-between",
              paddingTop: 32, borderTop: "1px solid var(--border)", width: "100%"
            }}>
              {[["7+","Years Experience"],["100K+","Users Served"],["73","Team Members Led"],["170+","KPI Dashboards Built"]].map(([num, label]) => (
                <div key={label} className="floating-stat" style={{ flex: "1 1 0", minWidth: 0 }}>
          ...
          <div ref={avatarRef} className="animate-in animate-in-d2 hide-mobile"
            onMouseMove={handleMove} onMouseLeave={handleLeave}
            style={{ position: "relative", perspective: 1400 }}>
            <div className="avatar-3d" style={{ transform: `perspective(1200px) rotateX(${tilt.y}deg) rotateY(${tilt.x}deg)`, position: "relative" }}>
              <div style={{
                width: "100%", height: 560, borderRadius: "4px 120px 4px 4px",
                ...
                <img
                  ...
                  alt="Md Nayeeb Rahman"
                  style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center top",
                    transform: "translateZ(20px) scale(1.02)", filter: "contrast(1.05) saturate(1.05)" }}
                />
              </div>
  ...
  return (
    <div style={{ borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", padding: "16px 0", overflow: "hidden", background: "var(--surface)" }}>
      <div className="marquee-track">
        {doubled.map((item, i) => (
          <span key={i} style={{ fontFamily: "var(--f-display)", fontSize: 18, letterSpacing: "0.08em", color: i % 4 === 0 ? "var(--volt)" : "var(--muted)", flexShrink: 0 }}>
...
const About = () => (
  <section id="about" style={{ padding: "120px 16px", maxWidth: 1800, margin: "0 auto", position: "relative" }}>
    ...
    <div className="reveal" style={{ marginBottom: 64 }}>
      ...
      <h2 className="display-lg">The <span className="serif-italic" style={{ color: "var(--volt)" }}>operator</span><br />behind the product.</h2>
    </div>
    <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 48 }}>
      <div>
        {[
          ...
          <p key={i} className={`reveal body-lg${i > 0 ? ` stagger-${Math.min(i,4)}` : ''}`} style={{ color: "var(--muted)", marginBottom: 28 }}>{text}</p>
        ))}
      ...
      <div className="reveal reveal-right" style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {[
          ...
          <div key={i} className={`floating-card card-3d stagger-${i+1}`} style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
            <div style={{ width: 40, height: 40, borderRadius: 4, background: "rgba(255,255,255,0.03)", border: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>{item.icon}</div>
            <div>
...
const Work = ({ scrollY }: { scrollY: number }) => {
  const jobs: { period: string; title: string; subtitle: string; company: string; location: string; color: ColorKey4; tags: string[]; highlights: string[] }[] = [
    {
      period: "Jan 2025 – Present", title: "Senior Business Analyst", subtitle: "Acting Senior Product Manager",
      company: "BRAC IT Services Ltd.", location: "Dhaka, Bangladesh", color: "volt",
    ...
    {
      period: "Mar 2023 – Dec 2024", title: "Business Analyst", subtitle: "Promoted from BA in 22 months — fastest in team",
      company: "BRAC IT Services Ltd.", location: "Dhaka, Bangladesh", color: "ice",
    ...
    {
      period: "Oct 2018 – Feb 2023", title: "Data Artist — Executive", subtitle: "Market Intelligence & Team Operations",
      company: "Catalyst Solutions (DBA Adiva Graphics)", location: "Dhaka & Chittagong", color: "ember",
    ...
    {
      period: "Consulting", title: "OTA Distribution Consultant", subtitle: "Flight & Property Channel Management",
      company: "Triploy.travel", location: "Remote", color: "magenta",
    ...
    <section id="work" style={{ padding: "120px 16px", background: "var(--surface)", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ maxWidth: 1800, width: "100%", margin: "0 auto", position: "relative" }}>
        ...
        <div className="reveal" style={{ marginBottom: 72 }}>
          ...
          <h2 className="display-lg" style={{ maxWidth: 760 }}>Seven years of <span className="serif-italic" style={{ color: "var(--volt)" }}>proof.</span></h2>
        </div>
          ...
          {jobs.map((job, i) => (
            <div key={i} className={`reveal experience-item stagger-${Math.min(i+1,4)}`} style={{ paddingBottom: 36 }}>
              <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 36, alignItems: "start" }}>
                <div>
                  <div style={{ fontSize: 11, color: "var(--muted)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 8 }}>{job.period}</div>
                  <div style={{ fontWeight: 700, fontSize: 12, color: colorMap4[job.color], letterSpacing: "0.08em" }}>{job.location}</div>
                </div>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: colorMap4[job.color], marginBottom: 16, fontWeight: 700, letterSpacing: "0.06em" }}>{job.subtitle}</div>
                  ...
                  <ul style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {job.highlights.map((h, j) => (
                      <li key={j} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                        <span style={{ color: colorMap4[job.color], flexShrink: 0, marginTop: 3 }}>→</span>
...
// ─── EXPERTISE ─────────────────────────────────────────────────────────────────
const Expertise = () => {
  const skillGroups: { title: string; color: ColorKey3; skills: [string, number][] }[] = [
    {
      title: "Product & AI Strategy", color: "volt",
      skills: [["Product Vision & Roadmap",95],["Backlog & Sprint Management",92],["User Stories & PRD Authoring",95],["Generative AI & Agentic Workflows",80],["Data Analytics (Python, SQL, KPI Systems)",92]]
    },
    {
      title: "OTA & Distribution", color: "ice",
      skills: [["Airbnb & Booking.com Extranets",85],["Rentals United / SiteMinder",80],["Rate Plan Configuration",82],["Channel Mix & Parity Management",78],["Distribution Dashboard Design",88]]
    },
    {
      title: "Marketing, Pre-Sales & Leadership", color: "magenta",
      skills: [["Pre-Sales & Solution Demos",88],["Market Research & Positioning",90],["Stakeholder & Client Management",92],["Team Leadership (up to 73 people)",94],["Coaching, 1:1s & Performance Reviews",90]]
    },
  ];
  ...
  return (
    <section id="expertise" style={{ padding: "120px 16px", maxWidth: 1800, margin: "0 auto", position: "relative" }}>
      ...
      <div className="reveal" style={{ marginBottom: 72 }}>
        ...
        <h2 className="display-lg">Where I <span className="serif-italic" style={{ color: "var(--volt)" }}>excel.</span></h2>
      ...
      <div className="grid-3" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 36 }}>
        {skillGroups.map((group, i) => (
          <div key={i} className={`reveal stagger-${i+1}`} style={{ display: "flex", flexDirection: "column", gap: 28 }}>
            <span className="tag" style={{ borderColor: `${colorMap3[group.color]}40`, color: colorMap3[group.color] }}>{group.title}</span>
            {group.skills.map(([name, pct]) => (
              <div key={name}>
                <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: "var(--text)" }}>{name}</span>
                  <span style={{ fontSize: 11, color: colorMap3[group.color], fontWeight: 700 }}>{pct}%</span>
                </div>
  ...
  <section id="projects" style={{ padding: "120px 16px", background: "var(--surface)", position: "relative" }}>
    <div style={{ maxWidth: 1800, width: "100%", margin: "0 auto" }}>
      ...
      <div className="reveal" style={{ marginBottom: 72 }}>
        ...
        <h2 className="display-lg soft-title" style={{ maxWidth: 820 }}>
          Building things that <span className="serif-italic" style={{ color: "var(--volt)" }}>don't exist yet.</span>
        </h2>
      </div>
      <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
        <div className="reveal card-3d" style={{ gridRow: "span 2", background: "var(--panel)", border: "1px solid rgba(200,255,0,0.15)", borderRadius: 4, padding: 48, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
          <div>
            <div style={{ width: 48, height: 48, borderRadius: 4, background: "rgba(200,255,0,0.1)", border: "1px solid rgba(200,255,0,0.2)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 24 }}><Sparkles size={22} style={{ color: "var(--volt)" }} /></div>
            ...
            <p className="body-md" style={{ color: "var(--muted)", marginBottom: 24 }}>An AI-first analytics and product studio building enterprise-grade Generative AI, Agentic AI, and RAG-based solutions. Our flagship product — NexNomad — is an AI-powered OTA platform enabling users to book hotels, flights, and trips with AI-assisted discovery.</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {["Generative AI pipelines for enterprise workflow automation","RAG-based document intelligence and compliance systems","NexNomad: AI-powered hotel, flight & trip OTA (in development)","Tax intelligence platform with agentic compliance review"].map((item, i) => (
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 32 }}>
            {["GenAI","Agentic AI","RAG","OTA Platform"].map(t => <span key={t} className="tag tag-volt">{t}</span>)}
          </div>
        </div>
        ...
        <div className="reveal stagger-1 card-3d" style={{ background: "var(--panel)", border: "1px solid rgba(168,216,255,0.12)", borderRadius: 4, padding: 36 }}>
          <div style={{ width: 44, height: 44, borderRadius: 4, background: "rgba(168,216,255,0.08)", border: "1px solid rgba(168,216,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}><Globe size={20} style={{ color: "var(--ice)" }} /></div>
          <span className="tag tag-ice" style={{ marginBottom: 12, display: "inline-block" }}>In Development</span>
          <h3 style={{ fontFamily: "var(--f-display)", fontSize: 32, marginBottom: 12 }} className="soft-title">NexNomad</h3>
          <p className="body-sm" style={{ color: "var(--muted)", marginBottom: 20 }}>AI-powered OTA platform where users can book hotels, flights, and trips using intelligent AI agents for discovery, comparison, and recommendation — with automated rate monitoring and dynamic pricing logic built in.</p>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {["OTA Architecture","Dynamic Pricing","AI Agents"].map(t => <span key={t} className="tag tag-ice">{t}</span>)}
          </div>
        </div>
        ...
        <div className="reveal stagger-2 card-3d" style={{ background: "var(--panel)", border: "1px solid rgba(255,61,170,0.12)", borderRadius: 4, padding: 36 }}>
          <div style={{ width: 44, height: 44, borderRadius: 4, background: "rgba(255,61,170,0.08)", border: "1px solid rgba(255,61,170,0.2)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}><TrendingUp size={20} style={{ color: "var(--magenta)" }} /></div>
          <span className="tag tag-magenta" style={{ marginBottom: 12, display: "inline-block" }}>Consulting</span>
          <h3 style={{ fontFamily: "var(--f-display)", fontSize: 32, marginBottom: 12 }} className="soft-title">Triploy.travel</h3>
          <p className="body-sm" style={{ color: "var(--muted)", marginBottom: 20 }}>OTA Distribution Consultant — owned the full channel function for Triploy's in-house flight OTA, from Rentals United and SiteMinder integrations to listing health dashboards and OTA account management.</p>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {["Rentals United","SiteMinder","Rate Parity"].map(t => <span key={t} className="tag tag-magenta">{t}</span>)}
          </div>
        </div>
      </div>
    </div>
...
const SocialProof = ({ scrollY }: { scrollY: number }) => (
  <section style={{ padding: "80px 16px", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", overflow: "hidden", position: "relative" }}>
    ...
    <div className="grid-4" style={{ maxWidth: 1800, width: "100%", margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 28, position: "relative", zIndex: 1 }}>
      {[
        { num: "100K+", label: "Platform Users Served", sub: "BRAC ERP, LMS, Agent Banking", color: "volt" as ColorKey3 },
        { num: "19",    label: "Largest Squad Led",     sub: "OneLMS cross-functional team", color: "ice" as ColorKey3 },
        { num: "170+",  label: "KPI Dashboards Built",  sub: "Zero manual reporting overhead", color: "magenta" as ColorKey3 },
        { num: "3",     label: "OTA Platforms",         sub: "Airbnb · Booking.com · Rentals United", color: "volt" as ColorKey3 },
      ].map((s, i) => (
        <div key={i} className={`reveal stagger-${i+1}`} style={{ textAlign: "left" }}>
          <div className="stat-num" style={{ color: colorMap3[s.color] }}>{s.num}</div>
    ...
    <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) minmax(520px, 680px)", gap: 72, alignItems: "start" }}>
      <div>
        <div className="reveal" style={{ marginBottom: 32 }}>
          ...
          <h2 className="display-lg soft-title" style={{ marginBottom: 24 }}>
            Ready to <span className="serif-italic" style={{ color: "var(--volt)" }}>build</span><br />something?
          </h2>
        ...
        <div className="reveal stagger-2" style={{ display: "flex", flexDirection: "column", gap: 16, marginBottom: 40 }}>
          {[
            ...
            <a key={i} href={item.href ?? undefined} target={item.href?.startsWith("http") ? "_blank" : undefined} rel="noopener noreferrer"
               className={`floating-card card-3d stagger-${i+1}`}
               style={{ display: "flex", gap: 16, alignItems: "center", textDecoration: "none", cursor: item.href ? "none" : "default" }}
               onMouseEnter={e => { if (item.href) (e.currentTarget as HTMLElement).style.borderColor = "rgba(200,255,0,0.3)"; }}
      ...
      <div className="reveal reveal-right" style={{ display: "flex", flexDirection: "column", gap: 16, minHeight: "100%" }}>
        <a href="mailto:nayeebrahman@gmail.com" style={{ background: "var(--volt)", borderRadius: 4, padding: 40, display: "flex", flexDirection: "column", gap: 16, textDecoration: "none", cursor: "none", boxShadow: "0 24px 60px rgba(200,255,0,0.16)" }} className="card-3d">
          <Mail size={28} style={{ color: "#000" }} />
          <h3 style={{ fontFamily: "var(--f-display)", fontSize: 36, color: "#000", lineHeight: 1 }}>Email Me Direct</h3>
          <p style={{ fontSize: 14, color: "rgba(0,0,0,0.65)", lineHeight: 1.6 }}>For job opportunities, consulting engagements, or project discussions. I respond within 24 hours.</p>
          <span style={{ display: "inline-flex", alignItems: "center", gap: 8, width: "fit-content", padding: "12px 24px", background: "#000", color: "#fff", fontFamily: "var(--f-body)", fontWeight: 800, fontSize: 12, letterSpacing: "0.08em", textTransform: "uppercase", borderRadius: 2 }}>
            nayeebrahman@gmail.com <ArrowUpRight size={14} />
          </span>
        </a>

        <div className="grid-2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <a href="https://wa.me/8801624373686" target="_blank" rel="noopener noreferrer" style={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 4, padding: 24, textDecoration: "none" }} className="card-3d">
          ...
          <a href="https://www.linkedin.com/in/md-nayeeb-rahman-b3a90a136/" target="_blank" rel="noopener noreferrer" style={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 4, padding: 24, textDecoration: "none" }} className="card-3d">
        ...
        <a href="tel:+8801787680648" style={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 4, padding: 24, textDecoration: "none", display: "flex", alignItems: "center", gap: 16 }} className="card-3d">
        ...
        <div className="contact-focus card-3d" style={{ marginTop: 8 }}>
          <div>
            ...
            <h3 style={{ fontFamily: "var(--f-display)", fontSize: 38, lineHeight: 0.95, color: "var(--soft-title)", marginBottom: 14 }}>
              Product systems that<br />ship, scale, and sell.
            </h3>
          ...
          <div className="focus-grid">
            ...
            <div className="contact-focus-card">
              ...
              <div style={{ color: "var(--text)", fontWeight: 700, fontSize: 14, lineHeight: 1.6 }}>Senior Manager<br />Lead PM / AI PM<br />CS Manager</div>
            </div>
...
const Footer = () => (
  <footer style={{ borderTop: "1px solid var(--border)", padding: "36px 16px" }}>
    <div style={{ maxWidth: 1800, width: "100%", margin: "0 auto", display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", gap: 24 }}>
      <div style={{ fontFamily: "var(--f-display)", fontSize: 28, color: "var(--volt)" }}>MNR.</div>
      <div style={{ fontSize: 11, color: "var(--muted)", letterSpacing: "0.1em", textTransform: "uppercase", textAlign: "center", whiteSpace: "nowrap" }}>
